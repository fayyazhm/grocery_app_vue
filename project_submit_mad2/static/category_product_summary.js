const category_product_summary=Vue.component("category_product_summary",{
    template:` 
    <div class="text-center" style="position:relative;top:140px;width: 90%;left: 60px;">
        <h1 style="text-decoration: underline;">{{ cat }}</h1>
        <table class="table">
  ...
  data(){
    return{
        product:"",ordl:"",total_pqty:"",total_aqty:"",dat:"",cat:"fay"
    }

  },
  props:["c"],
  mounted(){
    fetch(`/admin/summary/category/${this.c}`).then(response=>{return response.json()}).
    then((data)=>{
        ...
        this.dat=data["dat"]
        this.cat=data["cat"].category_name.toUpperCase()
...
export default category_product_summary