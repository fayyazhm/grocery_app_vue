const user_homepage= Vue.component("user_dashboard",{
    template:`
    <div>
      ...
      <div v-for="i in prod" v-if="c.category_id==i.product_category" class="col">
          ...
          <div class="input-group">
            <span class="input-group-prepend" >
                <button v-on:click="decrease(i.product_id)" class="btn btn-outline-secondary" style="background-color: mediumaquamarine;" type="button">-</button>
            </span>
            <input type="text" class="form-control text-center" v-bind:id="'input_'+i.product_id" v-model:value="cart_productqty[i.product_id]" style="background-color: lightgray;" readonly>
              <span class="input-group-append" >
                <button v-on:click="increase(i.product_id)" class="btn btn-outline-secondary" style="background-color: mediumaquamarine;" type="button">+</button>
              </span>
          </div>
          <button type="button" v-on:click="addcart(i.product_id)" class="btn btn-success green-button" style="--bs-btn-bg: mediumseagreen" >ADD TO <i class="bi bi-cart3" style="font-size:20px" ></i></button>	
      </div>
      <br>
      </div>   
    </div>
    ...
    mounted(){
        ...
        then((data)=>
        {
            this.user=data["user"]
            this.cat=data["cat"]
            this.prod=data["prod"].filter(i=>i.product_quantity>0)
            this.cart=data["cart"]
