const user_orders= Vue.component("user_orders",{
    template:`
    <div>
  ...
  mounted(){
    fetch(`/user/order/history/${this.c}`).then(response=>{return response.json()}).
    then(data=>{
      this.user=data['user']
      this.cart=data['cart']
      ...
      this.prod=data['prod']
      console.log(data)
      document.title= `${this.user.username.toUpperCase()} ORDERS`
